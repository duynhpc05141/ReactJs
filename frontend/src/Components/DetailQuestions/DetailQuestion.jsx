...
import { Button, Modal, notification, Image } from 'antd';
import './style.css';
...
function DetailQuestion() {
		...
		if (ref.current) {
			...
			codeBlocks.forEach((block) => {
				if (
					...
					button.innerHTML = `<span>${copyStatus}</span>`;
					button.addEventListener('click', () =>
						handleCopy(block.textContent),
					);
	...
	const handleDelete = (deletedId) => {
		setQuestionsReply((prev) =>
			prev.filter((q) => q._id !== deletedId)
		);
	};
			...
			<div className="pt-4 px-3">
				<div>
					<h2 className="text-xl uppercase mb-4">
						...
						<DropdownQ 
							postId={id} 
							postUserId={question?.user?._id} 
							onDelete={handleDelete}
						/>
					</h2>
					<div className="flex mb-1">
						<img
							src={question && question.user.avatar}
							className="rounded-full w-10 h-10"
							alt=""
						/>
						<Link to={`/author/${question && question.user._id}`}>
							<p className="ml-2">
			...
			{questionsReply?.map((r, index) => (
				<div className="pt-4 px-3" key={r._id}>
					<div>
						...
						<div className="flex mb-1">
							<img
								...
								alt=""
							/>
							<Link to={`/author/${r && r.user._id}`}>
								<p className="ml-2">{r && r.user.username}</p>
							</Link>
						</div>
						<hr />
						<br />
					...
					<div className="flex flex-wrap md:flex-nowrap justify-center">
						...
						<div className="flex-none w-[272px] md:block px-3"></div>
					</div>
					<p
