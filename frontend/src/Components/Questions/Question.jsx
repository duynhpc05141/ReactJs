	...
	useEffect(() => {
		const fetchQuestions = async () => {
			try {
				...
				const canDisplay = res.data.data.filter(
					(question) =>
						question.hide === false && question.reply.length === 0,
				);
		...
		<>
			<div className="pb-4 pt-4">
				<div className="pt-4 px-3">
					<h2 className="text-3xl uppercase mb-4">All Questions</h2>
								...
								<div className="flex flex-row justify-between  border-b-[1px] mt-3">
									...
									<div className="pb-5 mb-5 flex-[3] pl-2">
										<Link
											...
											className="mb-[6px] text-2xl hover:text-blue-500"
										>
										...
										<div className="flex gap-2">
											{question &&
												question.topic.map((tag) => (
													<Link
														to={`/tags/${tag._id}`}
														className="bg-gray-200 hover:bg-gray-300 rounded-lg p-1 text-sm"
													>
														{tag && tag.name}
													</Link>
												))}
									...
									questions.map(
										(q) =>
											...
											q.topic.map((tag) => (
												<div
													key={tag._id}
													className="ml-6 mb-3 text-sm bg-gray-200 hover:bg-gray-300 rounded-lg p-1 w-fit"
												>
													<Link
														to={`tags/${tag._id}`}
														className="bg-gray-200 hover:bg-gray-300 rounded-lg p-1 text-sm"
